...
final class PathParser {
  ...
  public static PVector<PathSegment> parse(String pathString) {
    String trimmed = pathString.trim();
    if (!trimmed.startsWith("$")) {
      throw new IllegalArgumentException("Path string must begin with $");
    }
    return parseRemaining(TreePVector.<PathSegment>empty(), trimmed.substring(1));
  }

  private static PVector<PathSegment> parseRemaining(PVector<PathSegment> parsed, String remaining) {
    if (remaining.isEmpty()) {
      return parsed;
    }

    if (remaining.startsWith(".*")) {
      return parseRemaining(parsed.plus(PathSegments.ofWildcard()), remaining.substring(2));
    }

    if (remaining.startsWith("[*]")) {
      return parseRemaining(parsed.plus(PathSegments.ofWildcard()), remaining.substring(3));
    }

    Matcher keyExprMatcher = keyExpr.matcher(remaining);
    if (keyExprMatcher.find()) {
      String key = keyExprMatcher.group().substring(1);
      return parseRemaining(parsed.plus(PathSegments.ofObjectKey(key)), remaining.substring(key.length() + 1));
    }
    ...
    if (deepKeyExprMatcher.find()) {
      ...
      return parseRemaining(parsed.plus(PathSegments.ofAny(key)), remaining.substring(key.length() + 2));
    }
    ...
    return parseRemaining(parsed.plus(parseIndexExpression(indexExpr)), remaining.substring(indexExpr.length() + 2));
  }
