...
import com.codepoetics.raffia.streaming.projecting.ArrayClosingProjector;
...
public final class StreamingWriters {
  ...
  public static <T extends BasketWriter<T>> FilteringWriter<T> filteringArray(Lens lens, T target) {
    return ArrayClosingProjector.closing(StreamingProjector.start(target.beginArray(), lens.getPath()));
  }
